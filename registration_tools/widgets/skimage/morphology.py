...
def get_skimage_morphology_functions():
    funcs = {}
    for name, func in inspect.getmembers(morphology, inspect.isfunction):
...
filter_func = get_skimage_morphology_functions()
...
class MorphologyWidget(QWidget):
    def __init__(self, viewer: napari.Viewer, filter_name: str):
        ...
        self.setWindowTitle(f"Morphology: {filter_name}")
        ...
        self.help_button.clicked.connect(lambda: webbrowser.open(f"https://scikit-image.org/docs/stable/api/skimage.morphology.html#skimage.morphology.{filter_name}"))
...
class MorphologySelector(QWidget):
    def __init__(self, viewer: napari.Viewer):
        super().__init__()
        self.viewer = viewer
        self.setWindowTitle("Skimage Morphology Selector")
        ...
        self.morphology = get_skimage_morphology_functions()
        self.dropdown.addItems(sorted(self.morphology.keys()))
        row.addWidget(self.dropdown)
        self.layout.addLayout(row)

        self.load_btn = QPushButton("Load Morphology UI")
        self.load_btn.clicked.connect(self.load_filter_ui)
        self.layout.addWidget(self.load_btn)

    def load_filter_ui(self):
        name = self.dropdown.currentText()
        func = self.morphology[name]
        widget = MorphologyWidget(self.viewer, name, func)
        self.viewer.window.add_dock_widget(widget, name=f"Morphology: {name}", area="right")
