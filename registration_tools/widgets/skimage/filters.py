...
default_map = {
    ("apply_hysteresis_threshold", "low"): 0.0,
    ("apply_hysteresis_threshold", "high"): 1.0,
    ("difference_of_gaussians", "low_sigma"): 1.0,
    ("gabor", "frequency"): 1
}
...
def get_skimage_filters_functions(funcs, module):
...
# filter_func = get_skimage_filters_functions()
...
class FilterWidget(QWidget):
    def __init__(self, viewer: napari.Viewer, filter_name: str):
        ...
        self.setWindowTitle(f"Filter: {filter_name}")
        ...
        self.help_button.clicked.connect(lambda: webbrowser.open(f"https://scikit-image.org/docs/stable/api/skimage.filters.html#skimage.filters.{filter_name}"))
...
class FilterSelector(QWidget):
    def __init__(self, viewer: napari.Viewer):
        super().__init__()
        self.viewer = viewer
        self.setWindowTitle("Skimage Filter Selector")
        ...
        self.filters = get_skimage_filters_functions()
        self.dropdown.addItems(sorted(self.filters.keys()))
        row.addWidget(self.dropdown)
        self.layout.addLayout(row)

        self.load_btn = QPushButton("Load Filter UI")
    ...
    def load_filter_ui(self):
        ...
        widget = FilterWidget(self.viewer, name, func)
        self.viewer.window.add_dock_widget(widget, name=f"Filter: {name}", area="right")
