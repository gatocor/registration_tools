    ...
    def test_registration(self):
        for dim,channels,dtype,stride,out_dataset,out_trnsf,direction,stepping in [
            ...
            with self.subTest(dim=dim, channels=channels, dtype=dtype, stride=stride, out_trnsf=out_trnsf, out_dataset=out_dataset, direction=direction, stepping=stepping):
                ...
                d = pd.DataFrame(columns=["pyramid_shrink_factor","pyramid_smoothing_sigmas","optimizer_radius","displacement_field_smoothing_sigmas","metric","score"])                
                for psf,pss in [
                    # [(1,),(0,)],
                    # [(1,),(1,)],
                    # [(1,),(2,)],
                    ...
                    for dfss in np.arange(0.5,3,0.5):
                        for opr in [
                            1,
                            # 2,
                            # 3,
                            ...
                            model = rt_reg.RegistrationSITK(
                                registration_type="TranslationTransform",
                                # registration_type="DisplacementFieldTransform",
                                # metric="ANTSNeighborhoodCorrelation",
                                # optimizer_radius=opr,
                                # displacement_field_smoothing_sigmas=dfss,
                                # pyramid_shrink_factor=psf,
                                # pyramid_smoothing_sigmas=pss,
                            )
                            img = model.fit_apply(
                                dataset,
                                direction=direction,
                            ...
                            s = model.score_registration(dataset[1], img[0], dataset.attrs["scale"], "MeanSquares")
                            d = pd.concat([d, pd.DataFrame([[psf,pss,opr,dfss,"MeanSquares",s]], columns=["pyramid_shrink_factor","pyramid_smoothing_sigmas","optimizer_radius","displacement_field_smoothing_sigmas","metric","score"])])

                d = d.sort_values(by=["score"], ascending=True)
                d.reset_index(drop=True, inplace=True)
                print(d)
                print(d.loc[0,"optimizer_radius"])
                ...
                model = rt_reg.RegistrationSITK(
                    ...
                    optimizer_radius=d.loc[0,"optimizer_radius"],
                    displacement_field_smoothing_sigmas=d.loc[0,"displacement_field_smoothing_sigmas"],
                    pyramid_shrink_factor=d.loc[0,"pyramid_shrink_factor"],
                    pyramid_smoothing_sigmas=d.loc[0,"pyramid_smoothing_sigmas"],
                )
